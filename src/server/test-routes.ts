...
export function registerTestRoutes(
  ...
  app.post('/api/__test__/member', async (c) => {
    ...
    const joinDate = body.join_date ?? '2026-01-01';
    const rawArchivedAt = body.archived_at ?? null;
    const archivedAt = rawArchivedAt ?? (body.status === 'archived' ? (getIstDate() < joinDate ? joinDate : getIstDate()) : null);

    if (!isValidYmdDate(joinDate)) {
      return c.json({ error: 'Invalid join_date format' }, 400);
    }
    if (archivedAt !== null && !isValidYmdDate(archivedAt)) {
      return c.json({ error: 'Invalid archived_at format' }, 400);
    }
    if (archivedAt !== null && archivedAt < joinDate) {
      return c.json({ error: 'archived_at must be on or after join_date' }, 400);
    }
